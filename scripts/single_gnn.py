...
# Description:
# Trains single-omics Graph Neural Network (TF-GNN / mt_albis) classifiers with Optuna tuning.
# Iterates over miRNA, RNA, and methylation feature subsets, building k-NN sample graphs
# with Gaussian-kernel edge weights, and evaluates each model on validation and test sets.
...
def clear_runtime_memory():
    """Best-effort memory cleanup for long-running HPC jobs."""
...
for selected_fs in ["mirnas", "rna", "methyl"]:
    ...
    results_folder = f"single/gnn/{fs_method}_{name}_{num_fs}/{selected_fs}"
    os.makedirs(results_folder, exist_ok=True)
    ...
    def train_model(model, graph_train, y_train, graph_val, y_val,
                    epochs=100, lr=3e-4, patience=15, class_weights=None):
        """
    ...
    def objective(trial):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X_tr_np, y_tr)):
            ...
            # Explicit cleanup between folds to avoid OOM in long Optuna runs.
            del model, graph_train, graph_val
            del X_fold_train, y_fold_train, X_fold_val, y_fold_val
            del cw, class_weights
            clear_runtime_memory()
    ...
    # Release large objects before training the next feature set model.
    del gnn_model, graph_tr, graph_val, study
    del X_tr_scaled, X_val_scaled, X_test2_scaled, X_test3_scaled
    del X_tr_np, X_val_np, X_test2_np, X_test3_np
    clear_runtime_memory()
