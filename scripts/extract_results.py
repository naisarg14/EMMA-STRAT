...
# Description:
#   Aggregates multi-omics model results by iterating over all combinations of
#   model, feature selection method, label, and feature count. Reads per-run
#   result text files and writes consolidated CSV tables (detailed and summary).

# ── Imports ───────────────────────────────────────────────────────────────────
import os

# ── Configuration ─────────────────────────────────────────────────────────────
codes = ["knn.py", "svm.py", "rf.py", "lgbm.py", "mlp.py", "gnn.py"]
...
def extract_results():
        ...
        for fs in fs_method:
            for label in labels:
                for num in features:
                    ...
                    count += 1
                    try:
                        result_folder = f"results2601/{code}/{fs}_{label}_{num}/"
                        common_data = {
                            "model": code,
                            "fs_method": fs,
                            "label": label,
                        ...
                        #print(f"\nExtracting results from {result_folder}")
                        with open(os.path.join(result_folder, "validation_set_results.txt"), "r") as val:
                            ...
                            for line in val_lines:
                                ...
                                if line.startswith("Balanced accuracy:"):
                                    ...
                                    auc = line.replace("MSS vs MSI-H", "").replace("AUC:", "").strip()
                                    try:
                                        auc = float(auc)
                                        results["AUC"] = auc
                                    except Exception:
                        ...
                        if label == "nt":
                            ...
                            continue
                        with open(os.path.join(result_folder, "test_set2_results.txt"), "r") as val:
                        ...
                        with open(os.path.join(result_folder, "test_set3_results.txt"), "r") as val:
    ...
    df.to_csv("model_results1002_msi.csv",index=False)
    ...
    summary_df = summary_df.rename(columns={
        ...
        "test2_macro_f1": "Test Set 2 Macro F1 Score",
        "test3_macro_f1": "Test Set 3 Macro F1 Score",
    })
    ...
    summary_df.sort_values(by=["Validation Macro F1 Score"], ascending=False, inplace=True)

    summary_df.to_csv("model_summary_results1002_msi.csv",index=False)
