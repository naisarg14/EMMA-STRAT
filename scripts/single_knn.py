...
# Description:
# Trains single-omics K-Nearest Neighbors classifiers with Optuna hyperparameter tuning.
# Iterates over miRNA, RNA, and methylation feature subsets independently,
# searching neighbor count and distance metrics via 5-fold CV and evaluating on test sets.
...
results_folder = f"single/knn/{fs_method}_{name}_{num_fs}"
...
for selected_fs in ["mirnas", "rna", "methyl"]:
    results_folder = f"single/knn/{fs_method}_{name}_{num_fs}/{selected_fs}"
    os.makedirs(results_folder, exist_ok=True)
    ...
    study = optuna.create_study(direction='maximize',
        storage=f"sqlite:///optuna_study_single.db",
        study_name=f"knn_single_optimization_{name}_{fs_method}_{num_fs}_{selected_fs}",
    )
