...
#   using Recursive Feature Elimination (RFE) backed by a Random Forest
#   classifier. Saves the top-k selected feature names to disk.
...
DATA_CONFIGS = {
    'methyl': {
        'csv_path': f'data_extraction/data/set1_methylation_model.csv',
        'feature_prefix': 'methyl',
        'target_features': target_features,
    },
    'mirna': {
        'csv_path': f'data_extraction/data/set1_mirna_model.csv',
        'feature_prefix': 'mirnas',
        'target_features': target_features,
    },
    'mrna': {
        'csv_path': f'data_extraction/data/set1_rna_model.csv',
        'feature_prefix': 'rna',
        'target_features': target_features,
    }
}
...
# ── Functions ─────────────────────────────────────────────────────────────────
def run_rferf_selection(data_type):
    """Run RFE with Random Forest for the specified data type."""
    ...
    for target_features in target_features_list:
        print(f"Processing TARGET_FEATURES = {target_features}")
        ...
        print(f"\nRunning RFE with Random Forest to select {target_features} features...")
        ...
        output_file = f"{folder}/selected_{feature_prefix}_RFERF_{target_features}.txt"
