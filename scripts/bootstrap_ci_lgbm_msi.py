...
#   AUC, and macro F1 of the best LightGBM MSI model (RFESVM, 20 features)
...
# ── Configuration ─────────────────────────────────────────────────────────────
MODEL_PATH = "results2601/lgbm/RFESVM_msi_20"
N_BOOTSTRAP = 10000  # Number of bootstrap samples
CONFIDENCE_LEVEL = 0.95
ALPHA = 1 - CONFIDENCE_LEVEL
os.makedirs("bootstrap_msi", exist_ok=True)
...
print("="*70)
print("Bootstrap 95% Confidence Intervals for LightGBM MSI Model")
print("Feature Selection: SVM RFE with 20 features")
...
def save_results(output_path, all_results):
    ...
    with open(output_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("Bootstrap 95% Confidence Intervals for LightGBM MSI Model\n")
        f.write("Feature Selection: SVM RFE with 20 features\n")
...
# ===== SAVE RESULTS =====
output_file = os.path.join("bootstrap", "bootstrap_confidence_intervals.txt")
...
for set_name, data in all_results.items():
    ...
    csv_path = os.path.join("bootstrap", f"bootstrap_distributions_{csv_name}.csv")
